...
import _Model.Price;
import _Model.StationUpdateSchedulprice;
...
public class ConvertParam {
				...
				for (Field ff : o.getClass().getDeclaredFields()) {
					...
					if (property_name.toUpperCase().equals(ff.getName().toUpperCase())) {
						if (property_type.equals(ff.getType())) {
							...
							if (property_type.equals(java.util.List.class)) {
	...
	public static void main(String[] args) {
		...
		price.departtime="2018-10-22";
		price.fullprice=22D;
		price.remainseats=22;
		List<Price> data = new ArrayList<Price>();
		data.add(price);
		List<StationUpdateSchedulprice> datas= new ArrayList<StationUpdateSchedulprice>();
		convert(data, datas);
	}
}
